...
# A matrix elements (corrected signs)
a23 = -(m_pend**2 * L_arm * l_pend**2 * g) / denom  # Added negative sign
a43 = (m_pend * l_pend * g * J_total) / denom       # Removed extra term
...
# --- 3. LQR Design ---
# Tuned weights for better performance
Q = np.diag([1.0, 0.1, 10.0, 0.1])  # Penalize: alpha, alpha_dot, theta, theta_dot
...
def non_linear_equations(state, u):
    """
    Computes the non-linear dynamics of the rotary inverted pendulum.
    
    Args:
        state: [alpha, alpha_dot, theta, theta_dot]
        u: control torque applied to the arm
    
    Returns:
        alpha_ddot, theta_ddot: angular accelerations
    """
    ...
    # Solve M * [alpha_ddot, theta_ddot]^T = C
    try:
...
def energy_controller(state):
    ...
    NOTE: theta = 0 is upright, so target energy is zero.
    """
    alpha, alpha_dot, theta, theta_dot = state
...
# --- 6. Hybrid Controller with Improved Switching ---
...
def system_dynamics(t, state):
    ...
    if near_upright:
        ...
        state_error = np.array([alpha, alpha_dot, theta_normalized, theta_dot])
        u_lqr = -K @ state_error
        u_lqr = float(u_lqr[0])
...
# --- 9. Run the Live Animation ---

print("\n" + "="*60)
print("Starting LIVE Rotary Inverted Pendulum Simulation")
print("="*60)
print("\nImprovements implemented:")
print("  ✓ Corrected linearization (fixed A matrix)")
...
print("\nSimulation stopped.")
print("\n=== Final Notes ===")
print("If the pendulum doesn't swing up properly:")
print("  1. Check energy pumping direction (try reversing sign in energy_controller)")
print("  2. Increase k_energy gain (try 1.5 or 2.0)")
print("  3. Verify A,B matrices match your system")
print("  4. Tune Q and R matrices for LQR")
print("  5. Adjust UPRIGHT_THRESHOLD and VELOCITY_THRESHOLD")